...
from utils.general import set_logging, increment_path, scale_coords, xyxy2xywh
from utils.plots import plot_one_box
from utils.torch_utils import select_device, time_synchronized

from obj_tracker import preprocessing, nn_matching
from obj_tracker.detection import Detection
...
names = []
dpath = 'images/arrays/'
dnames= [f for f in sorted(os.listdir(dpath))]

def update_tracks(tracker, frame_count, save_txt, txt_path, save_img, view_img, im0, gn, loaded_arr):
    ...
    for track in tracker.tracks:
        ...
        if save_img or view_img:  # Add bbox to image
            cords = (xyxy2xywh(torch.tensor(xyxy).view(1, 4))).view(-1).tolist()
            ptx,pty = cords[0], cords[1]
            dist = loaded_arr[int(pty)][int(ptx)]
            label = f'{class_name} #{track.track_id}~{int(dist)}'
            print(xyxy)
            plot_one_box(xyxy, im0, label=label,
                         color=get_color_for(label), line_thickness=opt.thickness)

def get_color_for(class_num):
    #diff colors (in hex) for bbox
...
def detect(save_img=False):
    ...
    for path, img, im0s, vid_cap in dataset:
        loaded_arr = np.loadtxt(dpath+dnames[tic])
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                # update tracks
                update_tracks(tracker, frame_count, save_txt, txt_path, save_img, view_img, im0, gn,loaded_arr)
